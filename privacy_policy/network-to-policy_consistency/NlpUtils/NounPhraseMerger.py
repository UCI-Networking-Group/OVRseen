...
def getLemma(nphrase):
    def getLemmaInt(tok):
        return tok.text.lower() if tok.lemma_ == '-PRON-' else tok.lemma_

    ##########################################
    def getLemmaSkipDetsAndPron(nphrase):
        return ' '.join([ getLemmaInt(w) for w in nphrase if w.pos != spacy.symbols.DET ])
...
def getEntType(np):
    ...
    if containOrgOrPers:
        return 'ORG'
    return most_common(ents) if len(ents) > 0 else ''
...
def mergeNounPhrasesDoc(doc, vocab, skipHeadWords=False):
    def mergeNounPhrasesInternal(nphrases, fixDeps=False):
        for np in nphrases:
            ...
            if skipHeadWords and len(np) > 1 and np[0].lemma_ in ['other', 'such', 'especially']: 
        ...
        def getPobjsEndIndex(token, startIndex):
            if isNoun(token):
                for ctok in token.children:
                    if ctok.dep == spacy.symbols.prep and ctok.lemma_ in ['of', 'on', 'in', 'about', 'regard']:
                        pobj = getPobj(ctok)
                        if pobj is not None:
        ...
        def getRelclEndIndex(token, endIndex):
            if isNoun(token):
                for child in token.children:
                    ...
                    if child.dep_ == 'relcl': #TODO check POS to ensure verb?
                        subj = getSubject(child)
                        if subj is not None and subj.lemma_ in ['that', 'which']: #Do we ever really have multiple subjects?
