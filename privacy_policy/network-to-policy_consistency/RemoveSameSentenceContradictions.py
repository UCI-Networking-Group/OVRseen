...
def dumpTable(conn, query, fname, directory, header):
    ...
    with open(os.path.join(directory, fname), 'w') as outputfile:
...
def dbToCsv(dbPath, output_dir):
    conn = sqlite3.connect(Path(dbPath).resolve(True).as_uri() + '?mode=ro', uri=True)
...
output_dir = DATA_ROOT / "combined_tables"
output_dir.mkdir(exist_ok=True)
dbToCsv(DATA_ROOT / 'output' / 'consistency_results.db', output_dir)

policyCsv = output_dir / "Policy.csv"
...
dataflowCsv = output_dir / "DataFlows.csv"
...
appDataflowCsv = output_dir / "AppDataFlows.csv"
...
policySentencesCsv = output_dir / "PolicySentences.csv"
...
consistencyResultCsv = output_dir / "ConsistencyResult.csv"
...
consistencyDataCsv = output_dir / "ConsistencyData.csv"
...
contradictionDataCsv = output_dir / "Contradiction.csv"
...
def writeCsvRow(csvfile, packageName, policyEntity, policyAction, policyData, policySentences, contradictionNum, contradictoryEntity, contradictoryAction, contradictoryData, contradictionSentences):
    csvfile.writerow((packageName, policyEntity, policyAction, policyData, policySentences if policySentences is not None else None, contradictionNum, contradictoryEntity, contradictoryAction, contradictoryData, contradictionSentences if contradictionSentences is not None else None))

#Let's just make a new CSV...
with open(DATA_ROOT / 'policylint_results.csv', 'w') as outputfile:
    csvfile = csv.writer(outputfile, delimiter=',')
...
dataOnt = nx.read_gml(DATA_ROOT / 'data' / 'data_ontology.gml')
entOnt = nx.read_gml(DATA_ROOT / 'data' / 'entity_ontology.gml')
...
contradictionInvMap = { v : k for k,v in contradictionMap.items() }
...
with open(output_dir / "ConsistencyData_wo_samesencontr.csv", 'w') as outputfile:
    csvfile = csv.writer(outputfile, delimiter=',')
    csvfile.writerow(header)
